...
def convex_hull_extend(clique, origins, dest, data, pariwise_mnap, visited_cliques):
    ...
    for trip in set(range(no_of_trips)) - clique:
        ...
        #![Hins] Filter out trips that are not shaerable
        is_trip_extendable = True
        for item in clique:
            if not pariwise_mnap[tuple(sorted((trip, item)))]:
                is_trip_extendable = False
                break
        if not is_trip_extendable:
            continue
...
def clique_generator(data, distances, max_diameter, connectivity_threshold):  
    ...
    # Pairwise_map is to buffer the result of can_serve_req
        ...
        for clique in prev_list:
            for trip in set(range(no_of_trips)) - clique:
                ...
                if can_serve_quasi_req(clique | {trip}, pairwise_map, connectivity_threshold):
                    ...
                    is_extended, extended_clique = convex_hull_extend(clique | {trip}, origins, dest, data, pairwise_map, visited_cliques)
                    if is_extended:
...
def clique_generator_on_map(H, max_diameter, distances, connectivity_threshold):
    ...
    # Pre-computation for efficiency
    #![Hins] Changing it for the real-world data
